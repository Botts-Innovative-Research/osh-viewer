    ...
    <v-card-text>
      <v-form @submit.prevent="handleSave">
        <v-select
          ...
          item-value="id"
        />
        <v-select v-model="selectedConfig" :item-props="itemProps" :items="configList"
          label="Select a configuration to save to" persistent-hint />
        <v-sheet v-if="selectedConfig === CREATE_NEW">
          <v-divider class="pb-4" />
          <h4 class="pb-4">Create a New Configuration</h4>
          <v-text-field v-model="newConfigName" label="Create new config name" persistent-hint
            :rules="[v => validateNewConfigName(v)]" class="pb-4" />
          <v-text-field v-model="newConfigDescription" label="Create new config description" persistent-hint
            :rules="[v => !!v || 'Config description is required']" />
        </v-sheet>
        <v-card-actions>
          <v-btn block type="submit" color="success" variant="tonal" :disabled="!canSave">Save
            State</v-btn>
        </v-card-actions>
      </v-form>
    </v-card-text>
