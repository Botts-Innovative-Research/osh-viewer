...
// Dynamically load the correct components based on selected type
const getStepComponent = (index: number) => {
  if (!vizwizStore.visualizationType) return null

  const base = `/src/viz/${vizwizStore.visualizationType}`

  if (index === 2) return defineAsyncComponent(() => import(`${base}/Config.vue`))
  if (index === 3) return defineAsyncComponent(() => import(`${base}/Customize.vue`))
      ...
      <template v-slot:default="{ prev, next }">
        <v-stepper-header>
          <template v-for="(step, index) in steps" :key="`${index}-step`">
            <v-stepper-item :complete="e1 > index + 1" :step="index + 1" :value="index + 1"
              :title="step.short"></v-stepper-item>
        ...
        <v-stepper-window>
          <v-stepper-window-item v-for="(step, index) in steps" :key="`${step.short}-content`" :value="index + 1">
            ...
            <component v-else-if="selectedType && (index === 2 || index === 3)" :is="getStepComponent(index)" />
...
<template v-slot:item.2>
        <v-card title="Select System & Datasource" flat>...</v-card>
      </template>

<template v-slot:item.3>
        <v-card title="Configure Properties" flat>...</v-card>
      </template>

<template v-slot:item.4>
        <v-card title="Customize Visualization" flat>...</v-card>
      </template>
