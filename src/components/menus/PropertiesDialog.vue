...
import { fetchCsSchema, fetchSchema, mineControlObsPropsFromCS, mineDatasourceObsPropsFromDS } from '@/lib/DatasourceUtils';
      ...
      <v-table>
        <tbody>
          ...
          <tr>
            <td>Properties</td>
            <td>
              <v-table density="compact" class="mt-1 mb-1">
                <tr v-for="property in schema">
                  <td>{{ property.label ?? property.name }}</td>
                  <td v-if="property.uom && property.uom.code">{{ property.uom.code }}</td>
                  <td v-else>-</td>
                </tr>
              </v-table>
            </td>
