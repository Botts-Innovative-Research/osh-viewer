...
export default function build() {
	console.log('Building Table Visualization...');
	const vizwizStore = useVizWizStore();
	const visualizationStore = useVisualizationStore();

	const datastreams = AggregateDatastreams(vizwizStore.dsConfig);

	const tableResult = CreateTableVizProps(
		datastreams,
		vizwizStore.visualizationCustomizationOptions
	);
	const visualizationComponents: VisualizationComponents = {
		dataSource: tableResult.vizDatasources,
		dataLayer: tableResult.dataLayer,
	};
	...
	console.log('Created Table Visualization:', newViz);
}

export function CreateTableVizProps(datastreams: { [key: string]: any }, visOptions: any) {
