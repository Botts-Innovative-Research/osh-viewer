...
watch(backgroundColor, (val) => {
	if (!lineId.value) vwStore.updateVisualizationCustomizationOptions({ backgroundColor: val });
	else {
	...
	// Set local backgroundColor based on store value
		...
		if (!lineId.value) {
			...
			backgroundColor.value =
				vwStore.visualizationCustomizationOptions.backgroundColor[lineId.value];
		}
	}
});
</script>
