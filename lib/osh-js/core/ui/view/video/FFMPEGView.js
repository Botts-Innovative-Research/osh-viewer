...
var FFMPEGView = /** @class */ (function (_super) {
            ...
            return __generator(this, function (_a) {
                if (!this.skipFrame) {
                    if (this.decodeWorker == null) {
                        console.log('[FFMPEGView] init worker', { codec: props.frameData && props.frameData.compression, dataSourceId: this.dataSourceId });
    ...
    FFMPEGView.prototype.initFFMPEG_DECODER_WORKER = function (codec) {
        console.log('[FFMPEGView] initFFMPEG_DECODER_WORKER start', { codec: codec });
        console.log('[FFMPEGView] worker url', './workers/ffmpeg.decode.video.worker.js');
...
//            this.decodeWorker = new DecodeWorker();
            ...
            console.error('[FFMPEGView] failed to construct worker', err);
            return;
        }
        // const drawWorker = new DrawWorker();
        this.decodeWorker.id = randomUUID();
        this.decodeWorker.onerror = function (err) {
        ...
        this.decodeWorker.onmessageerror = function (err) {
    ...
    FFMPEGView.prototype.decode = function (pktSize, pktData, timestamp, roll) {
        return __awaiter(this, void 0, void 0, function () {
            var dataView, safeData, arrayBuffer;
            return __generator(this, function (_a) {
                if (pktSize > 0) {
                    dataView = pktData instanceof Uint8Array ? pktData : new Uint8Array(pktData);
                    safeData = dataView.slice();
                    arrayBuffer = safeData.buffer;
                    this.decodeWorker.postMessage({
                        message: 'data',
